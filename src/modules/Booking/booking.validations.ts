...
import { vehicletype } from "./booking.constant";
 const BookingSchema =z.object({
    body:z.object({
        ...
        registrationPlate: z.string().min(1, "Registration plate is required").trim(),
      })
 });
 const UpdateBookingSchema =z.object({
    body:z.object({
        ...
        vehicleBrand: z.string().min(1, "Vehicle brand is required").trim().optional(),
        vehicleModel: z.string().min(1, "Vehicle model is required").trim().optional(),
        manufacturingYear: z
          .number()
          .int("Manufacturing year must be an integer")
          ...
          .max(new Date().getFullYear(), "Manufacturing year cannot be in the future")
          .optional(),
        registrationPlate: z.string().min(1, "Registration plate is required").trim().optional(),
      })
 });
